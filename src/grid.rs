...
impl Grid {
    ...
    pub fn cell_at(&mut self, pos: &Position) -> Option<&Cell> {
        // let map = self.map_mut();
        self.map.iter().find(|cell| cell.position == *pos)
    }

    pub fn link(&mut self, start: &Position, neighbour: &Position) {
        self.links.entry(*start).or_insert(vec![]).push(*neighbour);
    }

    pub fn is_linked(&self, start: &Position, pos: &Position) -> bool {
...
mod test {
    ...
    fn it_should_link_cells() {
        ...
        let next_cell_pos = container.grid.map[1].position;

        container
            .grid
            .links
            .entry(container.grid.map[0].position)
            .or_insert(vec![])
            .push(next_cell_pos);

        let next_cell_pos = container.grid.map[11].position;
        container
            .grid
            .links
            .entry(container.grid.map[7].position)
            .or_insert(vec![])
            .push(next_cell_pos);
        let next_cell_pos = container.grid.map[6].position;
        container
            .grid
            .links
            .entry(container.grid.map[7].position)
        ...
        assert_eq!(links, container.grid.links);
    }
...
"
        ...
        let next_cell_pos = grid.grid.map[1].position;

        grid.grid
            .links
            .entry(grid.grid.map[0].position)
            .or_insert(vec![])
            .push(next_cell_pos);

        let next_cell_pos = grid.grid.map[11].position;
        grid.grid
            .links
            .entry(grid.grid.map[7].position)
            .or_insert(vec![])
            .push(next_cell_pos);
